"""Pipeline for M-102 — Rad-ReStruct + OpenI structured chest X-ray VQA."""
...
class TaskPipeline(BasePipeline):
    ...
    def process_sample(self, raw_sample: dict, idx: int) -> Optional[TaskSample]:
        report_id = raw_sample["report_id"]
        ...
        intro_panel = render_panel(base, "Reviewing chest X-ray...", None, 0, len(chosen))
        first_q = render_panel(base, chosen[0]["question"], None, 0, len(chosen))
        first_a = render_panel(base, chosen[0]["question"], chosen[0]["answer"], 0, len(chosen))
        first_segment = (
        ...
        task_id = f"radrestruct_{report_id}_{idx:05d}"
        ...
        metadata = {
            "task_id": task_id,
            "source_dataset": "Rad-ReStruct + OpenI NLM CXR",
            "report_id": report_id,
    ...
    def run(self) -> List[TaskSample]:
        """Incremental-S3-upload run loop (mirrors M-25 pattern)."""
